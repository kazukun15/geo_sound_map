...
def export_csv(data):
    """
    スピーカー情報（[[lat, lon, label], ...]）を CSV に変換する関数。
    CSVの列は ["latitude", "longitude", "label"] とする。
    """
    rows = []
    for entry in data:
        if len(entry) == 3:
...
def compute_sound_grid(speakers, L0, r_max, grid_lat, grid_lon):
    """
    各スピーカーの音圧を計算し、全スピーカー分のパワーを合算して dB 値に変換する例。
    ...
    for spk in speakers:
        ...
        distance[distance < 1] = 1  # 最小1m
        # 単純な減衰モデル: L0 - 20 log10(distance)
        p_db = L0 - 20 * np.log10(distance)
        p_db[distance > r_max] = -999  # 最大距離以上は無効
        valid = p_db > -999
...
def cached_get_search_info(query: str) -> str:
    # 例として実装（実際のAPIエンドポイント、キーに合わせて修正）
