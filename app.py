...
# speakers: [ [lat, lon, [dir1_deg, dir2_deg]], ... ]
...
if st.session_state.heatmap_data is None:
    if len(st.session_state.speakers)==0:
        ...
        hd, iso60, iso80= calculate_heatmap_and_contours(
            st.session_state.speakers, L0, r_max, grid_lat, grid_lon
        )
...
# -------------------------------------------------------------
# 8) Folium地図表示
#    returned_objects=["center","zoom"] を指定 → ズーム操作も保存
# -------------------------------------------------------------
m= folium.Map(
